...
# Quick Time Machine Setup for Pi 5 (when Samba/Avahi already installed)
# This script completes the Time Machine setup
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  Quick Time Machine Setup${NC}"
echo -e "${BLUE}========================================${NC}"
echo
...
echo -e "${YELLOW}Detected: Samba and Avahi are already installed!${NC}"
echo
...
echo "- Check status: sudo systemctl status smbd avahi-daemon netatalk"
echo "- View logs: sudo tail -f /var/log/samba/log.smbd"
echo "- Test connection: smbclient -L localhost -U $MAC_USERNAME"
echo
