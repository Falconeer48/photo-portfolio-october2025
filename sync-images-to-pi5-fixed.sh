...
# Fixed sync-images-to-pi5-nested.sh
# Removed SSH key specification to use default SSH connection
...
if [ ! -d "$LOCAL_PATH" ]; then
    ...
    echo "Create it with: mkdir -p '$LOCAL_PATH'"
    exit 1
fi
...
check_pi5_space() {
    ...
    local available_kb
    available_kb=$(ssh "$PI_USER@$PI_HOST" "df -Pk \"$PI_PATH\" | awk 'NR==2 {print \$4}'") || return 1
    # Convert to bytes
...
ssh "$PI_USER@$PI_HOST" "find $PI_PATH -type d -mindepth 1 | sed \"s|$PI_PATH/||\" | sort"
...
for folder_path in "${portfolio_folders[@]}"; do
    ...
    if ! ssh "$PI_USER@$PI_HOST" "[ -d '$PI_PATH/$pi5_folder' ]"; then
        if [ "$DRY_RUN" = true ]; then
            echo "   🧪 [DRY RUN] Would create missing folder: $pi5_folder"
        else
            echo "   ➕ Creating missing folder: $pi5_folder"
            ssh "$PI_USER@$PI_HOST" "mkdir -p '$PI_PATH/$pi5_folder'"
        fi
    else
    ...
    if [ "$DRY_RUN" = true ]; then
        ...
        if rsync -avz --progress --update \
            ...
            if ssh "$PI_USER@$PI_HOST" "[ -f '$PI_PATH/$pi5_folder/Cover.jpg' ]"; then
                echo "   ✅ Cover.jpg already exists"
            else
                # Find the first image file in the folder
                first_image=$(ssh "$PI_USER@$PI_HOST" "find '$PI_PATH/$pi5_folder' -maxdepth 1 -type f \\( -iname '*.jpg' -o -iname '*.jpeg' -o -iname '*.png' \\) | head -1")
                
                if [ -n "$first_image" ]; then
                    # Rename the first image to Cover.jpg
                    echo "   📝 Renaming first image to Cover.jpg"
                    ssh "$PI_USER@$PI_HOST" "mv '$first_image' '$PI_PATH/$pi5_folder/Cover.jpg'"
                else
...
if [ "$sync_success" = true ]; then
    if [ "$DRY_RUN" = true ]; then
        ...
        if ssh "$PI_USER@$PI_HOST" "cd /home/ian/photo-portfolio && ./scripts/optimize-images.sh"; then
        ...
        ssh "$PI_USER@$PI_HOST" "sudo systemctl restart photo-portfolio.service && sleep 3"
